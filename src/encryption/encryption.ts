...
export const deriveDocumentKey = async(password: string, salt: Buffer | string) => {
...
export const encryptData = async (data: string, iv: Buffer | string, key: Buffer | string) => {
...
export const decryptData = async (encryptedData: string, iv: Buffer | string, key: Buffer | string) => {
    const ivBuffer = typeof iv === 'string' ? Buffer.from(iv, 'base64') : iv;
    const keyBuffer = typeof key === 'string' ? Buffer.from(key, 'base64') : key;
